...
LOGO_PATH = os.path.join(REPORTES_PDF_DIR, "logo.png")
...
class PDF(FPDF):
    ...
    def footer(self):
        ...
        self.cell(
            0,
            10,
            f"Página {self.page_no()} - {datetime.today().strftime('%Y-%m-%d %H:%M')}",
            0,
            0,
            "C",
        )
    ...
    def tabla(self, dataframe: pd.DataFrame):
        ...
        for col in dataframe.columns:
            max_content = max(
                dataframe[col].astype(str).apply(len).max(), len(str(col))
            )
...
def _save_or_return(pdf: FPDF, ruta_pdf: str | None):
...
def generar_pdf_apertura(df, ruta_pdf=None):
    ...
    pdf.cell(
        0,
        10,
        "Auditoría generada automáticamente por el sistema de inventario.",
        ln=1,
    )
    return _save_or_return(pdf, ruta_pdf)
...
def generar_pdf_cierre(df, ruta_pdf=None):
    ...
    pdf.cell(
        0,
        8,
        f"Total diferencia de stock (todas ubicaciones): {dif_total}",
        ln=1,
    )
    pdf.ln(2)
    pdf.tabla(df)
...
def generar_pdf_stock(df, ruta_pdf=None):
    ...
    pdf.cell(
        0,
        10,
        "Reporte de stock generado automáticamente por el sistema de inventario.",
        ln=1,
    )
    return _save_or_return(pdf, ruta_pdf)
